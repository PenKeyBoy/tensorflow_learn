        ...
        if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
            ...
            for e in range(config['epoch']):
                ...
                for image_tr,label_tr in dataManager.iterbatch(model.batch_size):
                    global_step,loss,_ = sess.run([model.global_step,model.loss,model.train_op],feed_dict={model.new_input:image_tr,model.tag:label_tr})
                    steps_per_epoch += 1
                    if e % 10 == 0:
                ...
                accuracy_value = sess.run(model.accuracy,feed_dict={model.new_input:dataManager.validation_images,model.tag:dataManager.validation_labels})
                ...
                if accuracy_value > maximum:
                    ...
                    test_accuracy_value = sess.run(model.accuracy,feed_dict={model.new_input:dataManager.test_images,model.tag:dataManager.test_labels})
                    logger.info("iteration %d: accuracy=%9.3f%%" %(e,test_accuracy_value*100))    #9:tab quantities
            xlim = (0,config['epoch']);ylim = (0,1)
            yticks = list(range(0,1,0.05))
